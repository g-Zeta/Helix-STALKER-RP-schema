ITEM.name = "Calibration Toolkit"
ITEM.description = "A professional set of tools for fine-tuning of equipment."
ITEM.longdesc = "The note on the container indicates that the set was made in Germany. The toolkit contains everything required for almost any kind of tuning and calibration work."
ITEM.model = "models/flaymi/anomaly/dynamics/equipments/quest/box_toolkit_3.mdl"
...
ITEM.soundWeapon = "stalker/inventory/inv_repair_kit_use_fast_2p8.mp3"
ITEM.soundArmor = "stalker/inventory/inv_repair_sewing_kit.mp3"

ITEM.isWeaponToolkit = true
...
ITEM.functions.use = { 
		...
		if (char) then
			...
			if (inv) then
				...
				for k, v in pairs(items) do
					local bIsWeapon = v.isWeapon
					local bIsArmor = (v.isBodyArmor or v.isArmor or v.isHelmet or v.isGasmask)

					if (bIsWeapon or bIsArmor) and v:GetData("equip") != true and item.repairTreshhold < (v:GetData("durability", 0)) and v:GetData("durability", 0) < 10000 then
    ...
    OnCanRun = function(item)				
		-- Requires both weapon and armor tech flags to use the expert kit
		local char = item.player:GetCharacter()

		if (char and char:HasFlags("7") and char:HasFlags("6")) then
    ...
    OnRun = function(item, data)
		...
		if target:GetData("equip") != true then
			...
			if char:HasMoney(cost) then
				...
				local sound = item.sound or "items/battery_pickup.wav"

				if (target.isWeapon) then
					sound = item.soundWeapon or sound
				elseif (target.isBodyArmor or target.isArmor or target.isHelmet or target.isGasmask) then
					sound = item.soundArmor or sound
				end

				client:EmitSound(sound)
				ix.chat.Send(client, "iteminternal", "uses their "..item.name.." to calibrate and repair the "..target.name..".", false)
			else
