...
function PANEL:Init()
	...
	self.realHeight = 200 * (ScrH() / 1080)
	self.height = 200 * (ScrH() / 1080)
	self:SetSize(250 * (ScrW() / 1920), 200 * (ScrH() / 1080))
	self:DockPadding(4 * (ScrW() / 1920), 4 * (ScrH() / 1080), 4 * (ScrW() / 1920), 4 * (ScrH() / 1080))
...
function PANEL:SetShowReset(value, name, default)
	...
	if (value and !IsValid(self.reset)) then
		...
		self.reset:DockMargin(4 * (ScrW() / 1920), 0, 0, 0)
...
function PANEL:AddCategory(name)
	...
	if (!IsValid(panel)) then
		...
		panel:SetText(name)
		panel:Dock(TOP)
		panel:DockMargin(0, 8 * (ScrH() / 1080), 0, 0)
