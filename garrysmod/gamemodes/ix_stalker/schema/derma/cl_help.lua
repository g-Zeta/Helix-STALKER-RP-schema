...
function PANEL:Init()
	...
	self.idlePanel:DockMargin(8 * (ScrW() / 1920), 0, 0, 0)
...
function PANEL:AddCategory(name)
	...
	panel:Dock(FILL)
	panel:DockMargin(8 * (ScrW() / 1920), 0, 0, 0)
	panel:GetCanvas():DockPadding(8 * (ScrW() / 1920), 8 * (ScrH() / 1080), 8 * (ScrW() / 1920), 8 * (ScrH() / 1080))
...
hook.Add("PopulateHelpMenu", "ixHelpMenu", function(tabs)
	...
	tabs["flags"] = function(container)
		...
		for k, v in SortedPairs(ix.flag.list) do
			...
			panel:Dock(TOP)
			panel:DockMargin(0, 0, 0, 8 * (ScrH() / 1080))
