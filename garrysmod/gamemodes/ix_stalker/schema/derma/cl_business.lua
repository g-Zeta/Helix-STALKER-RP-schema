...
function PANEL:Init()
	-- being relative.
	local size = 120
	self:SetSize(size * (ScrW() / 1920), size * 1.4 * (ScrH() / 1080))
end
...
function PANEL:SetItem(itemTable)
	...
	self.icon:Dock(FILL)
	self.icon:DockMargin(5 * (ScrW() / 1920), 5 * (ScrH() / 1080), 5 * (ScrW() / 1920), 10 * (ScrH() / 1080))
...
function PANEL:Init()
	...
	self:SetSize(1060 * (ScrW() / 1920), 761 * (ScrH() / 1080))
	self:SetPos(149 * (ScrW() / 1920), 80 * (ScrH() / 1080))

	self.categories = self:Add("DScrollPanel")
	self.categories:Dock(LEFT)
	self.categories:SetWide(260 * (ScrW() / 1920))
	...
	self.search:SetTall(36 * (ScrH() / 1080))
	self.search:SetFont("ixMediumFont")
	self.search:DockMargin(10, 0, 20, 9)
