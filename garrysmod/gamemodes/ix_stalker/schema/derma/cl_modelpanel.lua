...
function PANEL:Paint(w,h)
	...
	if not IsValid(self.Entity) or not IsValid(LocalPlayer()) or LocalPlayer():Health() <= 0 then return end
    ...
    local eyeHeight = 36
    local campos = self.Entity:GetPos() + fwd + Vector(0, 0, eyeHeight)
    local camtarget = self.Entity:GetPos() + Vector(0, 0, eyeHeight)

	cam.Start3D(campos, (camtarget - campos):Angle(), 30, x, y, w, h, 5, 4096)
	cam.IgnoreZ(false)
