...
function PANEL:Init()
	...
	createButton:SetText("Start a new story")
	...
	self.loadButton:SetText("Continue your adventure")
	...
	-- return button (closes the menu to return to the game)
	self.returnButton = self.mainButtonList:Add("ixMenuSelectionButton")
	self.returnButton:SetText("return")
	self.returnButton:SetBackgroundColor(ix.config.Get("color"))
	...
	-- leave button (disconnects from the server)
	self.leaveButton = self.mainButtonList:Add("ixMenuSelectionButton")
	self.leaveButton:SetText("Exit the Zone")
	self.leaveButton:SetBackgroundColor(derma.GetColor("Error", self))
	self.leaveButton:SizeToContents()
	self.leaveButton:SetButtonList(self.buttons)
	self.leaveButton.DoClick = function()
		local confirmText = L and L("Are you sure you want to leave?")
		Derma_Query(
			confirmText,
			L and L("You are leaving the Zone..."),
			L and L("Yes"), function()
	...
	self:UpdateReturnButton(self.bUsingCharacter)
end
...
function PANEL:Paint(width, height)
	derma.SkinFunc("PaintMainMenuBackground", self, width, height)
end
