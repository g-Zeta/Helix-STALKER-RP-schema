...
function PANEL:Init()
	...
	self.name:DockMargin(10 * (ScrW() / 1920), 5 * (ScrH() / 1080), 0, 0)
	self.name:Dock(TOP)
	self.name:SetTextColor(color_white)
	self.name:SetFont("stalkerregularfont")

	self.rep = self:Add("DLabel")
	self.rep:DockMargin(10 * (ScrW() / 1920), -3 * (ScrH() / 1080), 0, 0)
	self.rep:Dock(TOP)
	self.rep:SetTextColor(Color(138, 43, 226))
	self.rep:SetFont("stalkerregularsmallboldfont")

	self.description = self:Add("DLabel")
	self.description:DockMargin(10 * (ScrW() / 1920), 14 * (ScrH() / 1080), 0, 0)
...
function PANEL:Init()
	self:DockMargin(149 * (ScrW() / 1920), 0, 0, 10 * (ScrW() / 1920))
	self:SetTall(32)
...
function PANEL:Init()
	...
	self:SetSize(1205 * (ScrW() / 1920), 761 * (ScrH() / 1080))  -- Only set the desired height
	self:SetPos(0, 80 * (ScrH() / 1080))
