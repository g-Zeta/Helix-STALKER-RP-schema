...
function PLUGIN:EntityEmitSound(info)
	...
	if IsValid(info.Entity) and info.Entity:IsPlayer() and not IsEmittingPlayerStep and info.OriginalSoundName:find("step") and not info.OriginalSoundName:find("LandingSounds") then
		return false
	end
end
