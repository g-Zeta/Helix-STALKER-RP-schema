...
function ENT:Touch(activator)
	...
	if self.cooldown < CurTime() then
        local damageInfo = DamageInfo() -- Create a DamageInfo object
        damageInfo:SetDamage(10) -- Set the damage amount
        damageInfo:SetAttacker(self) -- Set the attacker (the entity itself)
        damageInfo:SetInflictor(self) -- Set the inflictor (the entity itself)
        damageInfo:SetDamageType(DMG_BURN) -- Set the desired damage type (change this to the type you want)

        activator:TakeDamageInfo(damageInfo) -- Apply the damage
